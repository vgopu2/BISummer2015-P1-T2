...
namespace AW.BusinessAccessLayer
{
    public abstract class TypeManager<T>
    {
        ...
        protected string MakeInsertClause(T filterEntity)
        {
            StringBuilder insertClause = new StringBuilder();
            //List<String> ls = new List<string>();
            //String.Join(" AND ", ls.ToArray());
            insertClause.Append("(");
            const string FILTER_FORMAT_STR = " '{0}'";
            const string FILTER_FORMAT_NUM = "  {0}";
            ...
            //insertClause.Remove(insertClause.Length - 1);
            insertClause.Length = insertClause.Length - 1;
            insertClause.Append(")");
            ...
            foreach (PropertyInfo pi in _propertyInfos)
            {
                ...
                if (value != null && !String.IsNullOrEmpty(value.ToString()))
                {
                    if (IsNumericType(pi.PropertyType))
                    {
                        if (value.ToString() != "0")
                        {
                            insertClause.Append(String.Format(FILTER_FORMAT_NUM,  value.ToString()));
                            insertClause.Append(",");
                        }
                    }
                    else
                    {
                        if (value.ToString() != "1/1/0001 12:00:00 AM" && value.ToString() != "00000000-0000-0000-0000-000000000000")
                        {
                            insertClause.Append(String.Format(FILTER_FORMAT_STR, value.ToString()));
                            insertClause.Append(",");
                        }
                    }
                }
